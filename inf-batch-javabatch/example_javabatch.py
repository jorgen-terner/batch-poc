...
#
#Viktigt: Glom inte att meddela AppDrift om forandringer sker i detta skript. De lyfter in skriptet manuellt under en overgangsperiod
#
def help_text():
    print('Script som startar ett batchjobb. Argument som kravs finns beskrivna nedan.')
    print('Mer information gar att hitta pa https://confluence.sfa.se/display/SOHDLS/Script\n')
    print('Exempel: python springbatch.py -j examplejob.json --start\n')
    print('Tvingande argument:')
    print('-j eller --job                                      (json-fil som pekar ut tjansterna samt url till miljo)n)')
    print('')
    print(' --start, --status, --restart, --stop eller --help  (Endast en av dessa kravs')
    print('')
    print('Frivillga argument:')
    print('-a eller --jobargs                               (argument som tjansten tar emot, i formatet "key=value")')
    print('--summary')
...
def main(argv):
    ...
    try:
        ...
        if help:
            print("Finns ingen hjalptjanst i jsonfilen, avslutar scriptet")
            sys.exit(1)
...
def prepare_and_run_monitor_script(method):
    if os.path.exists("/openprocess/scripts/rfvop/jbatch//monitor_jbatch.sh"):
        ...
        print("Monitor script hittades inte, fortsatter...")
...
# Dessa gar inte att kora pa windows, kors det inte i windows sa lagger jag aven till dessa.
if os.name != 'nt':

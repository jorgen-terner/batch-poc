...
class RemovePrefixOverlapTest {
    ...
    /**
     * Uses reflection to call the private removePrefixOverlap method from TemplateExecutionService.
     */
    private String removePrefixOverlap(String previousTail, String currentChunk) {
        try {
            var method = TemplateExecutionService.class.getDeclaredMethod("removePrefixOverlap", String.class, String.class);
            method.setAccessible(true);
            return (String) method.invoke(null, previousTail, currentChunk);
        } catch (Exception e) {
            throw new RuntimeException("Failed to invoke removePrefixOverlap via reflection", e);
        }
    }

    @Test
    void noOverlapShouldReturnFullChunk() {
        String result = removePrefixOverlap("hello", "world");
    ...
    void fullChunkOverlapShouldRemoveDuplicate() {
        ...
        String result = removePrefixOverlap("he\nllo\n", "llo\n");
        assertEquals("", result);
    }
    ...
    void partialOverlapAtLineBoundaryShouldDeduplicate() {
        ...
        String result = removePrefixOverlap("prefix\nline1\n", "line1\nline2\n");
        assertEquals("line2\n", result);
    }
    ...
    void partialOverlapNotAtLineBoundaryShouldNotDeduplicate() {
        ...
        String result = removePrefixOverlap("hel", "hello");
    ...
    void overlapAtCarriageReturnBoundaryShouldDeduplicate() {
        ...
        String result = removePrefixOverlap("data\r", "data\rmore");
    ...
    void multipleLinesWithOverlapShouldDeduplicate() {
        ...
        String result = removePrefixOverlap("line1\nline2\n", "line2\nline3\nline4\n");
        assertEquals("line3\nline4\n", result);
    }

    @Test
    void emptyCurrentChunkShouldReturnEmpty() {
        String result = removePrefixOverlap("previous", "");
        assertEquals("", result);
    }

    @Test
    void nullCurrentChunkShouldReturnEmpty() {
        String result = removePrefixOverlap("previous", null);
        assertEquals("", result);
    }

    @Test
    void nullPreviousTailShouldReturnFullChunk() {
        String result = removePrefixOverlap(null, "current");
        assertEquals("current", result);
    }

    @Test
    void emptyPreviousTailShouldReturnFullChunk() {
        String result = removePrefixOverlap("", "current");
        assertEquals("current", result);
    }

    @Test
    void bothEmptyShouldReturnEmpty() {
        String result = removePrefixOverlap("", "");
        assertEquals("", result);
    }
    ...
    void longOverlapWithMultipleLineBoundariesShouldFindFirstMatch() {
        ...
        String result = removePrefixOverlap("aaa\nbbb\nccc\n", "bbb\nccc\ndd");
    ...
    void singleCharacterOverlapShouldNotDeduplicate() {
        ...
        String result = removePrefixOverlap("a", "ab");
    ...
    void singleCharacterWithNewlineShouldDeduplicate() {
        // "\n" is a line boundary
        String result = removePrefixOverlap("a\n", "a\nb");
    ...
    void multipleOverlapCandidatesShouldUseConservativeMatch() {
        ...
        String previous = "x\ny\nz\n";
        String current = "y\nz\nmore";
        String result = removePrefixOverlap(previous, current);

...
### Docker-image
```bash
docker build -t trigger-job-api:1.0.0 .
```
...
```bash
helm install trigger-job-api ./helm/trigger-job-api
```

## Konfiguration

Se [Helm Chart README](../helm/trigger-job-api/README.md) för Kubernetes-specifika inställningar.
...
```bash
kubectl logs -f deployment/trigger-job-api
```
...
```bash
kubectl describe sa trigger-job-api
kubectl describe role trigger-job-api
```
